...
13. **分发执行与渠道**：DistributionAttempt 是发布/核对/重试的执行事实，Listing 只表示已知稳定远端身份；APP 没有 remoteId 时用标题永久 TM 核对，不准写 `MANUAL:TM...`。ChannelPrice 与 Item 默认价格分开，当前仅保存，不自动换算或覆盖 Item。分发 Token 仅限渠道内领取、读取当前包和回传结果，数据库只留哈希；UNKNOWN 必须核对原 Attempt，不能盲目重发。
