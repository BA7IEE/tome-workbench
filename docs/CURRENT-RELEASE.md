...
当前源码版本：**1.0.2-rc.1**，稳定化开发中，尚未完成发布验收。
...
- WebKit 登录根因已定位为原生 autofocus 延迟抢焦点，导致密码为空、浏览器原生校验阻止提交；修复后的 PR #2 最新 head 仍须按实时 CI 结果验收。
- 已实现版本统一、当前文档守卫、图片补偿、后端权限能力、生产备份恢复与 PushPlus 显式启用工具；完整发布验证进行中。UX 1.0.2 在独立分支实现本机选品草稿、图片排序、单次批量确认和状态主要动作，验收进行中。
