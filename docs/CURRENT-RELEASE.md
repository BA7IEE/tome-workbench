...
本来源成色降噪源码切片不新增 migration、历史数据写入或 Skill/Profile 变化。新候选不再写来源成色行动警告；旧提示仍存储并可追溯，但待确认卡片、表格和批量预检仅将 `conditionRaw` 作为普通来源参考显示。低置信度 Agent 建议、品牌未标准化、来源缺项和疑似重复仍是行动提示。本地成色仍须人工检查，见 [来源成色提示降噪](SOURCE-CONDITION-NOISE.md)。
...
本品牌治理源码切片不新增 migration，也不改 Agent Skill/Profile。字典管理员可只读取得待确认候选的品牌分组与逐件来源证据；人工确定标准品牌后，经现有字典接口维护标准项/别名，再用带显式候选与字典版本的独立批量命令绑定候选品牌。命令逐件锁定、复核、审计并写 Receipt，跳过已确认、排除、关联或冲突项；重导保留人工批准品牌。本分支没有读取或写入生产候选与字典；当期真实数据清单、业务批准、部署和回填均独立于源码验证，见 [品牌治理](BRAND-GOVERNANCE.md)。
