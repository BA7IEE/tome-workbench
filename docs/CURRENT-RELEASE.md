...
- WebKit 登录根因已定位为原生 autofocus 延迟抢焦点，导致密码为空、浏览器原生校验阻止提交；修复后的 PR #2 已通过本地完整门禁与远端 CI；合并状态仍以 GitHub 实时结果为准。
- 已实现版本统一、当前文档守卫、图片补偿、后端权限能力、生产备份恢复与 PushPlus 显式启用工具；本地完整门禁、CI 和打包结果以 VALIDATION 与 PR checks 为准。UX 1.0.2 在独立分支实现本机选品草稿、图片排序、单次批量确认和状态主要动作，验收结果以 VALIDATION 为准。
