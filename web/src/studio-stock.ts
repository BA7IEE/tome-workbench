...
export function studioStock(
  ...
  function paint() {
    ...
    if (host) {
      ...
      const direct: string[] = [];
      if (can("sell")) {
        if (i.status === "AVAILABLE") direct.push(button("记录询盘", () => recordInquiry(i, async () => {})));
      ...
      if (i.status === "AVAILABLE" && can("publish")) direct.push(`<a class="btn" href="#/items/${i.id}?tab=use&returnTo=${encodeURIComponent(location.hash)}">准备发布</a>`);
      ...
      host.innerHTML = i.id
        ? `<span class="studio-stock-badge">${esc(states[i.status] || i.status)}${i.dataMode === "TEST" ? " · 测试" : ""}</span>` +
          ...
          (can("sell")
            ? (i.status !== "AVAILABLE" && i.status !== "SOLD" ? button("记录询盘", () => recordInquiry(i, async () => {})) : "") + (canRecordSale(i) &&
              !["AVAILABLE", "RESERVED"].includes(i.status)
                ? button("登记售出", sold)
                : "") +
              (["AVAILABLE", "RESERVED"].includes(i.status)
