...
export async function sourcesScreen() {
  ...
  return (
    `<div id="${root}"><div class="page-title"><div><h1>货源与供应商</h1><p>维护手工供货资料与供应商；已接手商品在TM中经营，外部Agent资料统一到待确认处理。</p></div><div class="button-row"><a class="btn" href="#/items">手工录货</a><a class="btn primary" href="#/candidates">外部批量接收</a>${button("＋ 记录货源", () => singleSource(suppliers), "")}${button("导入表格", () => importSources(suppliers))}</div></div>` +
    section(
      "货源池",
