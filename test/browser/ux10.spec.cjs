...
test('快速录货只暴露首次需要的字段，完整建档退到更多',async({page})=>{
 ...
 await page.keyboard.press('Escape');await expect(page.getByRole('heading',{name:'快速录货'})).not.toBeVisible();await page.getByText('更多',{exact:true}).click();await expect(page.getByRole('link',{name:'完整建档',exact:true})).toBeVisible();
});
test('快速录货保存图片后直接下一件，上一件内容不带入',async({page})=>{
 ...
 await page.getByRole('button',{name:'保存并下一件',exact:true}).click();const next=page.getByRole('dialog',{name:'快速录货'});await expect(next).toBeVisible();await expect(next.getByText(/上一件 TM\d+ .*已保存/)).toBeVisible();await expect(next.getByLabel('商品名称',{exact:true})).toHaveValue('');await expect(next.getByLabel('品牌',{exact:true})).toHaveValue('');await expect(next.locator('.quick-intake-previews figure')).toHaveCount(0);
 const rows=await(await page.request.get('/api/items?q='+encodeURIComponent(title))).json();expect(rows.total).toBe(1);const item=await(await page.request.get('/api/items/'+rows.rows[0].id)).json();expect(item.assets).toHaveLength(1);expect(item.currentPrice).toBe(168000);expect(item.brand).toBe('Dior');
});
...
test('快速录货打开即聚焦名称，Ctrl+Enter保存并进入下一件',async({page})=>{
 const d=await openQuick(page),title='键盘录货 '+randomUUID().slice(0,8);await expect(d.getByLabel('商品名称',{exact:true})).toBeFocused();await d.getByLabel('商品名称',{exact:true}).fill(title);await page.keyboard.press('Control+Enter');const next=page.getByRole('dialog',{name:'快速录货'});await expect(next.getByLabel('商品名称',{exact:true})).toHaveValue('');const rows=await(await page.request.get('/api/items?q='+encodeURIComponent(title))).json();expect(rows.total).toBe(1);
});

test('快速录货可在提交前单张移除图片，服务器只保存保留项',async({page})=>{
 const d=await openQuick(page),title='图片取舍 '+randomUUID().slice(0,8);await d.getByLabel('商品名称',{exact:true}).fill(title);await d.getByLabel('商品图片').setInputFiles([await photo('keep.png'),await photo('remove.png')]);await expect(d.locator('.quick-intake-previews figure')).toHaveCount(2);await d.getByRole('button',{name:'移除图片 remove.png',exact:true}).click();await expect(d.locator('.quick-intake-previews figure')).toHaveCount(1);await d.getByRole('button',{name:'保存并下一件',exact:true}).click();const next=page.getByRole('dialog',{name:'快速录货'});await expect(next.getByLabel('商品名称',{exact:true})).toHaveValue('');const rows=await(await page.request.get('/api/items?q='+encodeURIComponent(title))).json(),item=await(await page.request.get('/api/items/'+rows.rows[0].id)).json();expect(item.assets.map(a=>a.originalName)).toEqual(['keep.png']);
});
