...
test('v1.3 标准 Agent 协议校验 Skill/Profile、显式纠错目录，且服务端 Profile 必查项不能被 Manifest 降低',async()=>{
  ...
  const weakBatch=await db.ingestBatch.create({data:{sessionId:session.id,procurementSourceId:source.id,externalBatchKey:'weak-'+suffix,agentName:'historical weak agent',agentVersion:'1.0',kind:'ORDER_HISTORY',rawManifest:{protocolVersion:'1.2',skillVersion:'tome-ingest/1.2',profile:'TRR/1.3',expectedCandidateKeys:['WEAK-'+suffix],requiredFields:['titleRaw']}}});
...
test('TRR/1.4 将展示尺码、标签原始尺码、估算标记与无时分秒购买日期分开，并保留TRR/1.3历史批次',async()=>{
  const suffix=randomUUID().slice(0,8).toUpperCase(),beforeItems=await db.item.count();
  const source=await ok('/procurement/sources','POST',{code:'TRR-'+suffix,name:'TRR尺码日期合成来源',kind:'MARKETPLACE',defaultCurrency:'USD'});
  const session=await ok('/ingest/sessions','POST',{procurementSourceId:source.id,label:'TRR/1.4 尺码日期',ttlMinutes:60});
  const protocol=await machineOk('/agent-ingest/protocol',session.token);
  assert.equal(protocol.profile.id,'TRR/1.4');
  const outdatedProfile=await machineApi('/agent-ingest/batches',session.token,'POST',{externalBatchKey:'trr13-new-'+suffix,agentName:'outdated TRR adapter',rawManifest:standardManifest('TRR/1.3')});
  assert.equal(outdatedProfile.status,400);assert.equal(outdatedProfile.data.error.code,'INGEST_PROFILE_MISMATCH');
  const batch=await machineOk('/agent-ingest/batches',session.token,'POST',{externalBatchKey:'trr14-'+suffix,agentName:'Synthetic TRR 1.4',rawManifest:standardManifest('TRR/1.4',{expectedCandidateKeys:['TRR14-'+suffix]})});
  const valid=structuredClone(goldenIngestFixture().candidates[0]);
  valid.externalKey='TRR14-'+suffix;valid.sourceItemKey='TRR14-SKU-'+suffix;
  const missingForeignSize=structuredClone(valid);
  delete missingForeignSize.sourceFacts.foreignSize;
  missingForeignSize.sourceFacts.capture.fields.find(field=>field.path==='sourceFacts.foreignSize').status='CAPTURED';
  const missingForeign=await machineApi(`/agent-ingest/batches/${batch.id}/candidates`,session.token,'POST',{candidates:[missingForeignSize]});
  assert.equal(missingForeign.status,400);assert.equal(missingForeign.data.error.code,'TRR_SOURCE_FACTS_INVALID');
  const timestamped=structuredClone(valid);
  timestamped.sourceFacts.order.orderedAt='2026-06-27T12:00:00.000Z';
  const badDate=await machineApi(`/agent-ingest/batches/${batch.id}/candidates`,session.token,'POST',{candidates:[timestamped]});
  assert.equal(badDate.status,400);assert.equal(badDate.data.error.code,'TRR_SOURCE_FACTS_INVALID');
  const proposalOverreach=structuredClone(valid);
  proposalOverreach.agentProposal.fields.push({path:'facts.foreignSize',value:'US 6',method:'INFERRED',confidence:0.2,evidencePaths:['sourceFacts.sizeLabel'],evidenceImageSha256:[],note:'不得把展示尺码冒充标签'});
  const blockedProposal=await machineApi(`/agent-ingest/batches/${batch.id}/candidates`,session.token,'POST',{candidates:[proposalOverreach]});
  assert.equal(blockedProposal.status,400);
  const imported=await machineOk(`/agent-ingest/batches/${batch.id}/candidates`,session.token,'POST',{candidates:[valid]});
  const current=await db.ingestCandidate.findUniqueOrThrow({where:{id:imported.rows[0].id}});
  assert.deepEqual(current.sourceFacts.order,{orderDateRaw:'June 27, 2026',orderedAt:'2026-06-27',datePrecision:'DAY'});
  assert.equal(current.sourceFacts.sizeLabel,'M');assert.equal(current.sourceFacts.foreignSize,'US 6');assert.equal(current.sourceFacts.sizeEstimated,true);
  assert.equal(current.decision,'PENDING');assert.equal(current.itemId,null);assert.equal(await db.item.count(),beforeItems);
  assert.equal((await machineOk(`/agent-ingest/batches/${batch.id}/seal`,session.token,'POST',{})).status,'SEALED');

  const legacyManifest=standardManifest('TRR/1.3',{expectedCandidateKeys:['TRR13-'+suffix]});
  const legacy=await db.ingestBatch.create({data:{sessionId:session.id,procurementSourceId:source.id,externalBatchKey:'trr13-'+suffix,agentName:'Historical TRR 1.3',agentVersion:'1.3',kind:'ORDER_HISTORY',rawManifest:legacyManifest}});
  const legacyRetry=await machineOk('/agent-ingest/batches',session.token,'POST',{externalBatchKey:legacy.externalBatchKey,agentName:'Historical TRR 1.3',agentVersion:'1.3',kind:'ORDER_HISTORY',rawManifest:legacyManifest});
  assert.equal(legacyRetry.id,legacy.id);assert.equal(legacyRetry.existing,true);
  const legacyCandidate=structuredClone(valid);
  legacyCandidate.externalKey='TRR13-'+suffix;legacyCandidate.sourceItemKey='TRR13-SKU-'+suffix;
  delete legacyCandidate.sourceFacts.foreignSize;delete legacyCandidate.sourceFacts.sizeEstimated;delete legacyCandidate.sourceFacts.order;
  ...
  const legacyImported=await machineOk(`/agent-ingest/batches/${legacy.id}/candidates`,session.token,'POST',{candidates:[legacyCandidate]});
...
test('外部Agent按目标字段选择、格式化并标注置信度，来源完整性与人工确认边界保持独立',async()=>{
  ...
  const imported=await machineOk(`/agent-ingest/batches/${batch.id}/candidates`,session.token,'POST',{candidates:[candidate]});
  const current=await ok(`/ingest/candidates/${imported.rows[0].id}`);assert.equal(current.sourceFacts.descriptionZh,undefined);assert.equal(current.proposal.category,'BAG');assert.equal(current.proposal.facts.descriptionZh,'根据合成来源资料整理的中文商品介绍。');assert.ok(current.warnings.some(message=>message.includes('外部Agent整理建议')));assert.ok(current.integrity.issues.some(message=>message.includes('合成来源没有二进制文件')));
  await machineOk(`/agent-ingest/batches/${batch.id}/seal`,session.token,'POST',{});assert.equal(await db.item.count(),before);
  const reviewed=await ok(`/ingest/candidates/${current.id}/review`,'POST',{version:current.version,possession:'UNKNOWN',decision:'PENDING',title:'合成结构手提包',category:'BAG',material:'小牛皮（人工复核）',color:'',sizeLabel:'Medium',measurements:'',descriptionZh:'人工复核后的合成中文商品介绍。',note:'修正存疑Agent建议'});
...
test('v1.3 薄 MCP 只复用 IngestService，和 HTTP 写出相同候选事实',async()=>{
  ...
  const mcpProtocol=await mcpTool(mcpSession.token,'tome_ingest_get_protocol');assert.equal(mcpProtocol.isError,false);assert.equal(mcpProtocol.value.profile.id,'TRR/1.4');
...
test('采购来源默认币种更正只影响后续缺省候选，3,422件TRR-1历史候选和成本事实保持不变',async()=>{
  ...
  const historical=await db.ingestBatch.create({data:{sessionId:session.id,procurementSourceId:source.id,externalBatchKey:'trr-1-historical-'+suffix,agentName:'Synthetic historical agent',agentVersion:'1.3',kind:'ORDER_HISTORY',rawManifest:standardManifest('TRR/1.3')}});
  ...
  const followup=await machineOk('/agent-ingest/batches',session.token,'POST',{externalBatchKey:'trr-1-followup-'+suffix,agentName:'Synthetic followup agent',rawManifest:standardManifest('TRR/1.4')});
  const followupCandidate=structuredClone(goldenIngestFixture().candidates[0]);
  followupCandidate.externalKey='TRR-1-FOLLOWUP-'+suffix;followupCandidate.sourceItemKey='TRR-1-FOLLOWUP-SKU';followupCandidate.titleRaw='TRR-1 后续缺省币种合成候选';delete followupCandidate.currency;
  const imported=await machineOk(`/agent-ingest/batches/${followup.id}/candidates`,session.token,'POST',{candidates:[followupCandidate]});
