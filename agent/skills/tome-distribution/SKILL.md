...
`X-Distribution-Token`，或同一 Token 的 `Authorization: Bearer ...`，按以下顺序调用：

1. `GET /api/distribution-agent/protocol`，取得当前会话指定的 Skill 与 Profile URL、ID 和
   SHA-256；
2. 下载两份 Markdown，先按原始字节计算 SHA-256，必须与协议一致才继续；没有精确
   Profile 时停止并请运营人员指定，不要自行猜测平台规则；
3. `tome_distribution_list_handoffs`，只处理返回的当前 Channel 记录；
4. 对要处理的 `recordId` 调用 `tome_distribution_get_package`。此操作把记录从
   “待交付”记为“已交付”，但不代表 ToMe 在外部平台执行任何步骤。

MCP 只有上述读取及两种结果回填工具；同一条记录的取包和回填都要传 12–128 位的
`idempotencyKey`。仅当一次 HTTP/MCP 写入的响应丢失时，才可用**原请求和原 key**取回
Receipt；不得因此重新取得资料、重做平台操作或另建交付。已交付超过时限或会话失效会
动态显示为 `ATTENTION`，只能人工按永久 TM 核对，绝不自动重发。不要把 Token、Cookie、
密码、验证码、真实账号或平台会话写入状态文件、日志、备注或回传内容。
...
当 `channel.platform = ANQICMS` 时，`platformData` 是
`{ "schema": "tome.anqicms/v1", "payload": ... }`，其 `payload` 复用已冻结的本地
AnQiCMS 合同，而不是外部 HTTP 请求或浏览器步骤。
...
APP 没有稳定编号时，`remoteId` 可留空；不得填写 `MANUAL:TM...` 等伪造 ID。AnQiCMS 的
`PUBLISH` 或 `UPDATE` 成功必须把真实稳定 `archive_id` 原样回传为 `remoteId`，空值、TM
或手工占位都会被拒绝。`DELIST` 使用同一工具表示停售动作已确认完成，不表示新建发布。
