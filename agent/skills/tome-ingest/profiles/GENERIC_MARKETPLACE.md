...
版本：`GENERIC_MARKETPLACE/1.2`。适用于没有专用 Profile 的市场来源。它规定字段语义，不规定网页 DOM、CSS selector 或浏览器自动化方式。
...
历史候选若误把别的金额写入 `sourceCurrentPrice`，可按标准 Skill 的受限 `sourceCorrection` 合同清空；必须同步提交该字段的 `UNAVAILABLE` 检查和来源侧原因。若整页字段或图片串到另一商品，可按协议白名单逐项清空、用 SHA-256 撤下尚未关联 TM 的错图，并作废旧 Agent 建议；原文件和审计仍保留。普通 `null` 仍是稀疏更新，不能删除已有值。
